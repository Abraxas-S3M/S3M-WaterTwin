"""Pluggable, read-only telemetry sources (shared ``ot_ingestion`` package).
...
source and any fallback are surfaced by the importing service (the watertwin-api
``/health`` + ``GET /api/v1/ingestion/source`` and the edge-gateway source-health
reporting).
...
calls (mirroring the platform's control-write boundary guard); the guard runs
from both ``services/watertwin-api`` and ``services/edge-gateway``.
"""
...
from .modbus import ModbusSource, RegisterSpec, parse_register_specs
...
logger = logging.getLogger("ot_ingestion.sources")
