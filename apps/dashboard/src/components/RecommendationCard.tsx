    ...
    <div className="rec-card" data-testid="recommendation-card">
      <div className="rec-top">
        ...
        <div className="row" style={{ gap: 6 }}>
          <span className={`status-chip ${rec.approval_status}`}>
            {t(`recommendation.status.${rec.approval_status}`)}
          </span>
          <ProvenanceBadge
      ...
      <div className="row" style={{ gap: 16 }}>
        <span className="card-sub">
          ...
          <strong style={{ color: 'var(--accent)' }}>{fmtNumber(rec.confidence * 100, 0)}%</strong>
        </span>
        {rec.asset_id ? (
          <span className="card-sub">{t('recommendation.asset', { id: rec.asset_id })}</span>
        ) : null}
        <span className="card-sub">
          {t('recommendation.mode', { mode: rec.control_boundary.control_mode })}
        </span>
      </div>

      <div>
        <div className="card-sub" style={{ marginBottom: 4 }}>{t('recommendation.rankedCauses')}</div>
        <ol className="causes">
      ...
      <div className="rec-action">
        <strong>{t('recommendation.recommendedAction')}</strong> {rec.recommended_action}
      </div>

      {rec.evidence?.assumptions?.length ? (
        <details>
          <summary className="card-sub">{t('recommendation.evidenceAssumptions')}</summary>
          <ul className="card-sub" style={{ margin: '6px 0 0', paddingLeft: 18 }}>
            <li>{t('recommendation.telemetryWindow', { window: rec.evidence.telemetry_window })}</li>
      ...
      {hasHandlers && !canDecide && (
        <div className="btn-row" data-testid="approve-role-gate">
          <span className="muted">
            <Trans i18nKey="recommendation.roleGate">
